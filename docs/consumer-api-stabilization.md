...
| Maintenance | Rust/Node/JavaScript suites, environment teardown, descriptor-lifetime tests, deterministic recovery-barrier injection, 25-cycle bounded soak, three-run large root recovery, and strict ordinary conformance cover the defining lifecycle. | Every support-state change requires the complete exact native/kernel matrix. Forced overflow remains a separately approved milestone gate. |
...
| 6 | Pass | Controlled source build and exact target packages, hashing, ELF checks, offline installation, lockstep validation, native ARM64, Nix, distro, Electron, release, kernel-floor, and supervised overflow evidence are complete. |
