...
| `features` | Recursive watching, moved-in discovery, subscription limits, process budget, shared native watches, overflow, exclusions, manual/automatic reconciliation, root recovery/qualification, bytes-only invalidations, exact bytes, ordered batches, observed state, cancellable establishment, and shared Node delivery. |
...
`rootQualification`, `bytesOnlyInvalidations`, `exactPathBytes`,
`orderedBatches`, `observedState`, `cancellableEstablishment`, and
`sharedNodeDelivery`.

Schema 8 adds `ChangeBatch.pathEncoding` with `complete`, `root-collapsed`, and
`bytes-only` states. Establishment buffers the one admitted delivery until the
physical root namespace is known. A bytes-only batch has no string root
fallback; exact physical bytes remain authoritative.
...
`>=24.15.0 <25`. ARMv7 has no published usable binding yet, and its declared
execution evidence is emulated rather than native-hardware evidence.
