...
| --- | --- | --- | --- |
| GNU/Linux x64 | `@gadicc/watchbound-node-linux-x64-gnu` | Node `>=18.15.0`, Node-API 6+, kernel 5.15+, glibc 2.35+ | [✅ Supported](docs/support-matrix.md#source-candidate-target-contract) |
| GNU/Linux ARM64 | `@gadicc/watchbound-node-linux-arm64-gnu` | Node `>=18.15.0`, Node-API 6+, kernel 5.15+, glibc 2.35+ | [✅ Supported](docs/support-matrix.md#source-candidate-target-contract) |
| GNU/Linux ARMv7 hard-float | `@gadicc/watchbound-node-linux-arm-gnueabihf` | Node `>=18.15.0`, Node-API 6+, ARMv7-A hard-float little-endian glibc, kernel 5.15+ | [✅ Supported](docs/support-matrix.md#source-candidate-target-contract) |

The supported native targets remain deliberately narrow. Detected WSL and environments with recognized container evidence cannot qualify. Musl, soft-float or unknown 32-bit ARM ABIs, big-endian ARM, and non-Linux platforms fail closed.

Node support is not a tested-version allowlist. The JavaScript API requires Node 18.15.0 or newer, the native addon requires process Node-API 6 or newer, and exact CI versions are evidence points. The loader has no Node upper bound, but it retains exact target selection and all platform, ARM ABI, libc, kernel, ELF, digest, metadata, and capability checks. See the [Node and Electron compatibility policy](docs/runtime-compatibility.md).

CI also exercises pinned Ubuntu, Debian, Fedora, Arch x64, openSUSE, Nix, kernel 5.15, representative Node and Electron releases, an ARMv7 QEMU-user Electron lifecycle lane, and a real ARMHF 5.15 kernel under system QEMU. ARMv7 execution evidence is emulated and makes no native-hardware or performance claim. The [full support and qualification matrix](docs/support-matrix.md) records exact lanes, architectures, artifacts, and promotion evidence; immutable registry and release records identify what a particular published version contains.
